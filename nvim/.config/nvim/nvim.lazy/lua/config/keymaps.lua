...
map('n', "<leader>/", ":lua require('Comment.api').toggle.linewise.count(vim.v.count > 0 and vim.v.count or 1)",
  { desc = "Toggle comment line" })
map('v', "<leader>/", "<esc>:lua require('Comment.api').toggle.linewise(vim.fn.visualmode())<cr>",
  { desc = "Toggle comment line" })
-- better indents
map("v", "<", "<gv")
map("v", ">", ">gv")
