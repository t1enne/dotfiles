...
- `x-request-id`: unique UUID per request (generate with $(uuid))
- `x-api-key`: Public API Key (<ETORO_PUBLIC_KEY>)
- `x-user-key`: User Key (<ETORO_API_KEY>)
...
## Practical Examples & Known Issues

### Environment Variables

Credentials are typically stored in environment variables:

```bash
ETORO_PUBLIC_KEY  # Public API Key (application key)
ETORO_API_KEY     # User Key (account key)
```

### Generating UUIDs

The `uuid` command may not be available on all systems. Use one of these alternatives:
...
### Example: Get Demo Portfolio Positions

```bash
UUID=$(cat /proc/sys/kernel/random/uuid) && curl -s -X GET "https://public-api.etoro.com/api/v1/trading/info/demo/portfolio" \
  -H "x-request-id: $UUID" \
  -H "x-api-key: $ETORO_PUBLIC_KEY" \
  -H "x-user-key: $ETORO_API_KEY" | jq .
```

**Response includes:**

- `positions[]` - open positions with `positionID`, `instrumentID`, `isBuy`, `amount`, `leverage`, `openRate`, `units`, `takeProfitRate`, `stopLossRate`
- `orders[]` - pending orders with `orderID`, `rate` (limit price), `instrumentID`
- `credit` - available balance

### Example: Search for an Instrument

```bash
UUID=$(cat /proc/sys/kernel/random/uuid) && curl -s -X GET "https://public-api.etoro.com/api/v1/market-data/search?internalSymbolFull=SILVER&fields=instrumentId,internalSymbolFull,displayname" \
  -H "x-request-id: $UUID" \
  -H "x-api-key: $ETORO_PUBLIC_KEY" \
  -H "x-user-key: $ETORO_API_KEY" | jq .
```

**Tip:** Search returns multiple results for futures/variants. Look for "Non Expiry" instruments for standard trading:

- `SILVER` (instrumentId: 19) = "Silver (Non Expiry)" - the main silver instrument
- `SILVER.APR26`, `SILVER.MAY26`, etc. are futures contracts

### Example: Get Current Prices

```bash
UUID=$(cat /proc/sys/kernel/random/uuid) && curl -s -X GET "https://public-api.etoro.com/api/v1/market-data/instruments/rates?instrumentIds=19,100001" \
  -H "x-request-id: $UUID" \
  -H "x-api-key: $ETORO_PUBLIC_KEY" \
  -H "x-user-key: $ETORO_API_KEY" | jq .
```
...
- `lastExecution` - last trade price
- `date` - timestamp of the quote

### Example: Get Instrument Metadata

```bash
UUID=$(cat /proc/sys/kernel/random/uuid) && curl -s -X GET "https://public-api.etoro.com/api/v1/market-data/instruments?instrumentIds=19,100001&fields=instrumentId,internalSymbolFull,instrumentDisplayName" \
  -H "x-request-id: $UUID" \
  -H "x-api-key: $ETORO_PUBLIC_KEY" \
  -H "x-user-key: $ETORO_API_KEY" | jq .
```
...
1. **Real Portfolio Permission Error**
   - `/trading/info/portfolio` may return `InsufficientPermissions` even with a valid key
   - This happens when your key is for a **Virtual/Demo** environment
   - Use `/trading/info/demo/portfolio` for demo accounts
   - Each User Key is tied to either Real OR Demo, not both

2. **Key Environment Matters**
   - A "Virtual" key can ONLY access demo endpoints
   - A "Real" key can ONLY access real endpoints
   - Check your key's environment in eToro Settings > Trading
